...
public class ServerAdapter extends RecyclerView.Adapter<ServerAdapter.OriginalViewHolder> {
    ...
    @Override
    public void onBindViewHolder(final ServerAdapter.OriginalViewHolder holder, @SuppressLint("RecyclerView") final int position) {

        CommonModels obj = items.get(position);
        holder.name.setText(obj.getTitle().toLowerCase());
        ...
        if (position == 0 && type.equals("tv")) {
            ...
            holder.name.setTextColor(ctx.getResources().getColor(R.color.grey_40));
